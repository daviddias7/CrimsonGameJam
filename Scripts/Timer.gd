...
func _on_timeout():
	if indicator:
		...
		wait_time = randf_range(0.2 + min_time, max_time)
	enemy.attack(indicator)
	indicator = !indicator
	
func spawn_snowball(player_position):
	var snowball_instance = snowball_scene.instantiate()
	snowball_instance.position = get_parent().get_parent().position
	snowball_instance.get_node("RigidBody2D").linear_velocity = BALL_SPEED * ((player_position - snowball_instance.position).normalized() + Vector2(randf_range(-1,1), randf_range(-1,1)).normalized() * desvio_angular)
