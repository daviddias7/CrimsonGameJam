...
func _on_timeout():
	...
	snowball_instance.get_node("RigidBody2D").linear_velocity = BALL_SPEED * (get_parent().get_parent().get_parent().get_node("Player").position - snowball_instance.position).normalized()
